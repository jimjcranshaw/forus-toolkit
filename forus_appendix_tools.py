...
def draw_a1(c, data=None):
    ...
    for col_x, (tcol, ttitle, tdesc, tinds) in zip(col_xs, tiers):
        ...
        c.drawString(col_x+2*mm, y, 'SIGNS THIS IS YOUR ROLE'); y -= 5*mm
        for ind in tinds:
    ...
    c.drawString(M+4*mm, ab_y+16*mm,
        'ONCE YOU HAVE IDENTIFIED YOUR TIER — COMMUNICATE IT TO MEMBERS IN WRITING')
    _tb(c, d["A1_OUTCOME_BOX"], M+4*mm, ab_y+10*mm, CW-8*mm, 'Helvetica', 8, col=WHITE, ld=10)
    _ftr(c, 'Appendix A1 of 3  ·  Linked from B2 — Solidarity Mechanisms')


# ── Appendix A2 ───────────────────────────────────────────────────────────────

def draw_a2(c, data=None):
    d = dict(A2_DEFAULTS)
    if data: d.update(data)
    ...
    c.drawString(M, s1y, 'STEP 1  —  THE READINESS GATE')

    s1bh = 46*mm; s1by = s1y - 6*mm - s1bh
    c.setFillColor(OFFWHITE); c.roundRect(M, s1by, CW, s1bh, 2*mm, fill=1, stroke=0)
    c.setFillColor(DKGREY); c.setFont('Helvetica', 8)
    c.drawString(M+4*mm, s1by+s1bh-7*mm, 'Tick each statement that is true for your organisation today:')
    ...
    s2y = ob_y - 9*mm
    c.setFillColor(TEAL); c.setFont('Helvetica-Bold', 9)
    c.drawString(M, s2y, 'STEP 2  —  MUTUALISATION READINESS  (complete only if proceeding)')
    c.setFillColor(MIDGREY); c.setFont('Helvetica', 7.5)
    c.drawString(M, s2y-6*mm, 'Tick each item. A single gap should trigger a planning conversation before you commit.')

    cw2 = (CW - 5*mm) / 2; s2cs = s2y - 14*mm
    ly = _draw_sect(c, M, s2cs, 'TRUST', TEAL,
        [d["A2_TRUST_ITEM_1"], d["A2_TRUST_ITEM_2"]], cw2)
    ly = _draw_sect(c, M, ly, 'GOVERNANCE', MINT,
        [d["A2_GOVERNANCE_ITEM_1"], d["A2_GOVERNANCE_ITEM_2"]], cw2)
    ry = _draw_sect(c, M+cw2+5*mm, s2cs, 'CAPACITY', DKGREY,
...
def draw_a3(c, data=None):
    ...
    c.setFillColor(OFFWHITE); c.roundRect(M, ty-ib_h, CW, ib_h, 2*mm, fill=1, stroke=0)
    c.setFillColor(TEAL); c.setFont('Helvetica-Bold', 8.5)
    c.drawString(M+4*mm, ty-6*mm, 'HOW TO USE')
...
def build_appendix_pdf(selected_ids, data=None, buf=None):
